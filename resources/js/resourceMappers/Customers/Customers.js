...
export const userDrawable = {
    created_at: function(item) {
        let datetime = moment(item.created_at).format(dateTimeFormat()).split(' ')
    ...
    subscription_ends_on: function(item) {
        ...
        let datetime = moment(item.subscription_ends_on).format(dateTimeFormat()).split(' ')

        let date = datetime[0] + datetime[1]
        let time = datetime[2] + datetime[3]

        return `<span>${date}</span> <span class='text-gray-5   00'>${time}</span>`
    }
}
